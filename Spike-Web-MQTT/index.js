...
if (exports.enabled) {
    ...
    var client  = mqtt.connect(mqtt_broker, { clientId: "mqttjs01" })
    console.log("connected flag  " + client.connected);


    //console.log("XDSFAFDTrying to connect to..." + mqtt_broker )
    //console.log('saved ip: ' + exports.settings.ip);
    //console.log('saved port: ' + exports.settings.port);
...
function startHardwareInterface() {
    ...
    server.addNode(objectName, TOOL_NAME, "stopMotors", "node", {x: 0, y: 125, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "color", "node", {x: 75, y: -175, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "distance", "node", {x: 0, y: -175, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "force", "node", {x: -75, y: -175, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "accelerometerX", "node", {x: -125, y: -100, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "accelerometerY", "node", {x: -125, y: -25, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "accelerometerZ", "node", {x: -125, y: 50, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "gyroscopeX", "node", {x: -200, y: -100, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "gyroscopeY", "node", {x: -200, y: -25, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "gyroscopeZ", "node", {x: -200, y: 50, scale:0.25});

    // Adds motor nodes to the object on the app
    server.addNode(objectName, TOOL_NAME, "motor1", "node", {x: 125, y: -100, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "motor2", "node", {x: 125, y: -25, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "motor3", "node", {x: 125, y: 50, scale:0.25});
    server.addNode(objectName, TOOL_NAME, "CustomProgram", "node", {x: 150, y: -175, scale:0.25});
    ...
    if (complexity == 'beginner' || complexity == 'intermediate') {
        ...
        if (complexity == 'beginner') {
            ...
            server.addNode(objectName, TOOL_NAME, "screen", "node", {x: -125, y: -25, scale:0.25});
            server.addNode(objectName, TOOL_NAME, "LED", "node", {x: -125, y: 50, scale:0.25});
            server.addNode(objectName, TOOL_NAME, "motors", "node", {x: 125, y: -25, scale:0.25});
    ...
    server.addReadListener(objectName, TOOL_NAME, "motor1", function(data){
        ...
        if (runMotors) {
            setTimeout(() => { publish(run_topic, "motor1.pwm(" + Math.round(data.value) + ")\r\n") }, 0);
        }
    ...
    server.addReadListener(objectName, TOOL_NAME, "motor2", function(data){
        // If we are running motors, then run the motor at the speed of the value sent to the node
        if (runMotors) {
            setTimeout(() => { publish(run_topic, "motor2.pwm(" + Math.round(data.value) + ")\r\n") }, 0);
        }
        // Else stop the motors
        else {
            stopMotors()
    ...
    server.addReadListener(objectName, TOOL_NAME, "motor3", function(data){
        // If we are running motors, then run the motor at the speed of the value sent to the node
        if (runMotors) {
            setTimeout(() => { publish(run_topic, "motor3.pwm(" + Math.round(data.value) + ")\r\n") }, 0);
        }
        // Else stop the motors
        else {
            stopMotors()
